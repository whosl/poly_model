...
def normalize_shadow_config(raw_cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized = {
        **cfg,
        ...
        "realtime_archive": {
            **(cfg.get("realtime_archive") or {}),
            "enabled": bool((cfg.get("realtime_archive") or {}).get("enabled", True)),
            # 1s feature/state snapshots are the most useful retraining unit for
            # terminal models while keeping storage bounded.
            "snapshot_interval_seconds": float((cfg.get("realtime_archive") or {}).get("snapshot_interval_seconds", 1.0)),
            "max_time_to_expiry_seconds": float((cfg.get("realtime_archive") or {}).get("max_time_to_expiry_seconds", 180.0)),
            "include_feature_json": bool((cfg.get("realtime_archive") or {}).get("include_feature_json", True)),
            "include_orderbook_json": bool((cfg.get("realtime_archive") or {}).get("include_orderbook_json", True)),
            "primary_latency_ms": int((cfg.get("realtime_archive") or {}).get("primary_latency_ms", 1000)),
            "drawdown_event_threshold_total": float((cfg.get("realtime_archive") or {}).get("drawdown_event_threshold_total", 25.0)),
            "recent_outcomes_in_drawdown_event": int((cfg.get("realtime_archive") or {}).get("recent_outcomes_in_drawdown_event", 50)),
        },
        "signal_queue": {
...
class ShadowRuntime:
    def __init__(self, cfg: dict[str, Any], args: argparse.Namespace) -> None:
        ...
        self.realtime_snapshot_sink = ParquetBuffer(base_shadow / "realtime_market_snapshots", "realtime_market_snapshots")
        self.signal_context_sink = ParquetBuffer(base_shadow / "signal_context", "signal_context")
        self.drawdown_sink = ParquetBuffer(base_shadow / "drawdown_events", "drawdown_events")
        ...
        self.last_realtime_snapshot_ts: dict[str, datetime] = {}
        self.primary_pnl_total_cum = 0.0
        self.primary_pnl_total_peak = 0.0
        self.primary_max_drawdown_total = 0.0
        self.last_drawdown_event_level = 0.0
        self.recent_primary_outcomes: deque[dict[str, Any]] = deque(maxlen=250)
    ...
    def _maybe_emit_signal(self, market_id: str, recv_ts: datetime, window_id: str | None = None) -> None:
        ...
        self._record_realtime_snapshot(market_id, recv_ts, feature_map, p_up, p_down, p_flat)
    ...
    def _orderbook_payload(self, st: MarketState) -> dict[str, Any]:
        return {
            "yes_bid_levels": st.yes.bid_levels[:10],
            "yes_ask_levels": st.yes.ask_levels[:10],
            "no_bid_levels": st.no.bid_levels[:10],
            "no_ask_levels": st.no.ask_levels[:10],
            "yes_event_lag_seconds": st.yes.event_lag_seconds,
            "no_event_lag_seconds": st.no.event_lag_seconds,
            "yes_ts_event": st.yes.ts_event,
            "yes_ts_recv": st.yes.ts_recv,
            "no_ts_event": st.no.ts_event,
            "no_ts_recv": st.no.ts_recv,
        }

    def _compact_market_context(self, market_id: str, recv_ts: datetime, feature_map: dict[str, Any]) -> dict[str, Any]:
        st = self.market_states[market_id]
        return {
            "market_id": market_id,
            "sample_ts": recv_ts,
            ...
            "btc_open_price": st.btc_open_price,
            "btc_bid": self.binance.bid,
            "btc_ask": self.binance.ask,
            "btc_mid": self.binance.mid,
            "btc_bid_qty": self.binance.bid_qty,
            "btc_ask_qty": self.binance.ask_qty,
            "btc_ts_event": self.binance.ts_event,
            "btc_ts_recv": self.binance.ts_recv,
            ...
            "no_event_lag_seconds": st.no.event_lag_seconds,
            "pair_ask_sum": feature_map.get("pair_ask_sum"),
            "seconds_since_last_pm_update": feature_map.get("seconds_since_last_pm_update"),
        }

    def _record_realtime_snapshot(self, market_id: str, recv_ts: datetime, feature_map: dict[str, Any], p_up: float, p_down: float, p_flat: float) -> None:
        cfg = self.cfg.get("realtime_archive", {})
        if not bool(cfg.get("enabled", True)):
            return
        tte = self._num(feature_map.get("time_to_expiry_seconds"))
        max_tte = self._num(cfg.get("max_time_to_expiry_seconds"))
        if tte is None or (max_tte is not None and tte > max_tte):
            return
        interval = float(cfg.get("snapshot_interval_seconds", 1.0))
        last = self.last_realtime_snapshot_ts.get(market_id)
        if last is not None and (recv_ts - last).total_seconds() < interval:
            return
        self.last_realtime_snapshot_ts[market_id] = recv_ts
        feature_payload = {name: feature_map.get(name) for name in self.feature_names}
        ctx = self._compact_market_context(market_id, recv_ts, feature_map)
        row = {
            "run_id": self.run_id,
            "date": recv_ts.date().isoformat(),
        ...
        if bool(cfg.get("include_feature_json", True)):
            row["feature_json"] = json.dumps(feature_payload, default=str, separators=(",", ":"))
        if bool(cfg.get("include_orderbook_json", True)):
            row["orderbook_json"] = json.dumps(self._orderbook_payload(self.market_states[market_id]), default=str, separators=(",", ":"))
        self.realtime_snapshot_sink.append(row)
    ...
    def _emit_signal(
        ...
        self._record_signal_context(signal_row, feature_map)
    ...
    def _record_signal_context(self, signal_row: dict[str, Any], feature_map: dict[str, Any]) -> None:
        cfg = self.cfg.get("realtime_archive", {})
        if not bool(cfg.get("enabled", True)):
            return
        market_id = str(signal_row["market_id"])
        recv_ts = signal_row["sample_ts"]
        st = self.market_states[market_id]
        row = {
            **signal_row,
            "context_write_ts": utc_now(),
            "model_type": self.model_type,
            "target_shares": float(self.cfg["model"].get("target_shares", 1.0)),
            "fee_rate": float(self.cfg["model"].get("fee_rate", 0.07)),
            "slippage_buffer": float(self.cfg["model"].get("slippage_buffer", 0.0025)),
            "btc_bid": self.binance.bid,
            "btc_ask": self.binance.ask,
            "btc_bid_qty": self.binance.bid_qty,
            "btc_ask_qty": self.binance.ask_qty,
            ...
            "pair_ask_sum": feature_map.get("pair_ask_sum"),
            "seconds_since_last_pm_update": feature_map.get("seconds_since_last_pm_update"),
            "feature_json": json.dumps({name: feature_map.get(name) for name in self.feature_names}, default=str, separators=(",", ":")),
            "orderbook_json": json.dumps(self._orderbook_payload(st), default=str, separators=(",", ":")),
            "binance_recent_mid_json": json.dumps([(ts.isoformat(), v) for ts, v in list(self.binance.mid_hist)[-120:]], separators=(",", ":")),
            "pm_recent_yes_mid_json": json.dumps([(ts.isoformat(), v) for ts, v in list(st.pm_mid_hist)[-120:]], separators=(",", ":")),
        }
        self.signal_context_sink.append(row)
    ...
    def _advance_pending(self, market_id: str, recv_ts: datetime) -> None:
        ...
        for po in self.pending:
            ...
            if po.exit_price is not None or recv_ts > po.exit_due_ts + timedelta(seconds=2):
                ...
                self.outcomes_sink.append(outcome_row)
                self._record_drawdown_state(outcome_row, recv_ts)
            else:
                remaining.append(po)
        self.pending = remaining

    def _record_drawdown_state(self, outcome_row: dict[str, Any], recv_ts: datetime) -> None:
        cfg = self.cfg.get("realtime_archive", {})
        if not bool(cfg.get("enabled", True)):
            return
        primary_latency = int(cfg.get("primary_latency_ms", 1000))
        if int(outcome_row.get("entry_latency_ms") or -999) != primary_latency:
            return
        net_total = self._num(outcome_row.get("net_pnl_total"))
        if net_total is None:
            return
        self.primary_pnl_total_cum += net_total
        self.primary_pnl_total_peak = max(self.primary_pnl_total_peak, self.primary_pnl_total_cum)
        drawdown = self.primary_pnl_total_cum - self.primary_pnl_total_peak
        self.primary_max_drawdown_total = min(self.primary_max_drawdown_total, drawdown)
        compact = {
            "signal_id": outcome_row.get("signal_id"),
            "market_id": outcome_row.get("market_id"),
            "direction": outcome_row.get("direction"),
            "signal_ts": outcome_row.get("signal_ts"),
            "entry_price": outcome_row.get("entry_price"),
            "exit_price": outcome_row.get("exit_price"),
            "net_pnl_total": net_total,
            "cum_pnl_total": self.primary_pnl_total_cum,
            "drawdown_total": drawdown,
        }
        self.recent_primary_outcomes.append(compact)
        threshold = abs(float(cfg.get("drawdown_event_threshold_total", 25.0)))
        if threshold <= 0:
            return
        # Emit an event every time drawdown crosses another threshold bucket.
        level = math.floor(abs(drawdown) / threshold) * threshold
        if drawdown < 0 and level > self.last_drawdown_event_level:
            self.last_drawdown_event_level = level
            keep = max(1, int(cfg.get("recent_outcomes_in_drawdown_event", 50)))
            self.drawdown_sink.append(
                {
                    "run_id": self.run_id,
                    "date": recv_ts.date().isoformat(),
                    ...
                    "cum_pnl_total": self.primary_pnl_total_cum,
                    "peak_pnl_total": self.primary_pnl_total_peak,
                    "drawdown_total": drawdown,
                    "max_drawdown_total": self.primary_max_drawdown_total,
                    "trigger_signal_id": outcome_row.get("signal_id"),
                    "trigger_market_id": outcome_row.get("market_id"),
                    "trigger_direction": outcome_row.get("direction"),
                    "trigger_net_pnl_total": net_total,
                    "recent_outcomes_json": json.dumps(list(self.recent_primary_outcomes)[-keep:], default=str, separators=(",", ":")),
                }
            )
    ...
    def finalize(self) -> None:
        ...
        self.realtime_snapshot_sink.flush()
        self.signal_context_sink.flush()
        self.drawdown_sink.flush()
    ...
    try:
        while utc_now() < rt.stop_wall:
            ...
            if int(time.time()) % flush_interval == 0:
                ...
                rt.realtime_snapshot_sink.flush()
                rt.signal_context_sink.flush()
                rt.drawdown_sink.flush()
    finally:
        for t in tasks:
            t.cancel()
